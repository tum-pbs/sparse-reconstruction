...
def get_setup_3d(sim_type:str, is_test_set:bool, sim_id:int):
    if not is_test_set:
        seed_name = sim_type
    else:
...
def initial_condition_generator(p:dict, varied:dict) -> ex.ic.BaseRandomICGenerator:
    ...
    if init_type == "Truncated Fourier":
        ...
        ic_gen = ex.ic.RandomTruncatedFourierSeries(3, cutoff=cutoff, max_one=True)
        ...
        ic_gen = ex.ic.GaussianRandomField(3, powerlaw_exponent=powerlaw, max_one=True)
        ...
        ic_gen = ex.ic.DiffusedNoise(3, intensity=intensity, max_one=True)
...
def get_advection(is_test_set:bool, seed:int) -> tuple[dict, dict, ex.BaseStepper, jnp.ndarray]:
    p = {
        ...
        "Constants": ["Velocity X", "Velocity Y", "Velocity Z"],
        "Initial Types": ["Truncated Fourier", "Gaussian Field", "Diffused Noise"],
        "Initial Frequency Cutoff (min)": 2,
        "Initial Frequency Cutoff (max)": 9,
        "Initial Powerlaw Exponent (min)": 2.5,
        "Initial Powerlaw Exponent (max)": 7,
        "Initial Noise Intensity (min)": 0.0001,
    ...
    vel = np.random.uniform(p["Velocity (min)"], p["Velocity (max)"], 3)
    stepper = ex.stepper.Advection(3, p["Domain Extent"], p["Resolution"], p["Dt"]/p["Sub Steps"], velocity=vel)

    varied["Velocity X"] = vel[0]
    varied["Velocity Y"] = vel[1]
    varied["Velocity Z"] = vel[2]
...
def get_diffusion(is_test_set:bool, seed:int) -> tuple[dict, dict, ex.BaseStepper, jnp.ndarray]:
    p = {
        ...
        "Boundary Conditions Order": ["x negative", "x positive", "y negative", "y positive", "z negative", "z positive"],
        "Boundary Conditions": ["periodic", "periodic", "periodic", "periodic", "periodic", "periodic"],
        "Constants": ["Viscosity X", "Viscosity Y", "Viscosity Z"],
        "Initial Types": ["Truncated Fourier", "Gaussian Field", "Diffused Noise"],
        "Initial Frequency Cutoff (min)": 2,
        "Initial Frequency Cutoff (max)": 9,
        "Initial Powerlaw Exponent (min)": 2.5,
    ...
    nu = np.random.uniform(p["Viscosity (min)"], p["Viscosity (max)"], 3)
    stepper = ex.stepper.Diffusion(3, p["Domain Extent"], p["Resolution"], p["Dt"]/p["Sub Steps"], diffusivity=nu)

    varied["Viscosity X"] = nu[0]
    varied["Viscosity Y"] = nu[1]
    varied["Viscosity Z"] = nu[2]
...
def get_advection_diffusion(is_test_set:bool, seed:int) -> tuple[dict, dict, ex.BaseStepper, jnp.ndarray]:
    p = {
        ...
        "Boundary Conditions Order": ["x negative", "x positive", "y negative", "y positive", "z negative", "z positive"],
        "Boundary Conditions": ["periodic", "periodic", "periodic", "periodic", "periodic", "periodic"],
        "Constants": ["Viscosity X", "Viscosity Y", "Viscosity Z", "Velocity X", "Velocity Y", "Velocity Z"],
        "Initial Types": ["Truncated Fourier", "Gaussian Field", "Diffused Noise"],
        "Initial Frequency Cutoff (min)": 2,
        "Initial Frequency Cutoff (max)": 9,
        "Initial Powerlaw Exponent (min)": 2.5,
    ...
    nu = np.random.uniform(p["Viscosity (min)"], p["Viscosity (max)"], 3)
    vel = np.random.uniform(p["Velocity (min)"], p["Velocity (max)"], 3)
    stepper = ex.stepper.AdvectionDiffusion(3, p["Domain Extent"], p["Resolution"], p["Dt"]/p["Sub Steps"], velocity=vel, diffusivity=nu)
...
def get_dispersion(is_test_set:bool, seed:int) -> tuple[dict, dict, ex.BaseStepper, jnp.ndarray]:
    p = {
        ...
        "Boundary Conditions Order": ["x negative", "x positive", "y negative", "y positive", "z negative", "z positive"],
        "Boundary Conditions": ["periodic", "periodic", "periodic", "periodic", "periodic", "periodic"],
        "Constants": ["Dispersivity X", "Dispersivity Y", "Dispersivity Z"],
        "Initial Types": ["Truncated Fourier", "Gaussian Field", "Diffused Noise"],
        "Initial Frequency Cutoff (min)": 2,
        "Initial Frequency Cutoff (max)": 9,
        "Initial Powerlaw Exponent (min)": 2.5,
    ...
    dispersivity = np.random.uniform(p["Dispersivity (min)"], p["Dispersivity (max)"], 3)
    stepper = ex.stepper.Dispersion(3, p["Domain Extent"], p["Resolution"], p["Dt"]/p["Sub Steps"], dispersivity=dispersivity)

    varied["Dispersivity X"] = dispersivity[0]
    varied["Dispersivity Y"] = dispersivity[1]
    varied["Dispersivity Z"] = dispersivity[2]
...
def get_hyper_diffusion(is_test_set:bool, seed:int) -> tuple[dict, dict, ex.BaseStepper, jnp.ndarray]:
    ...
    stepper = ex.stepper.HyperDiffusion(3, p["Domain Extent"], p["Resolution"], p["Dt"]/p["Sub Steps"], hyper_diffusivity=diffusivity)
...
def get_burgers(is_test_set:bool, seed:int) -> tuple[dict, dict, ex.BaseStepper, jnp.ndarray]:
    ...
    stepper = ex.stepper.Burgers(3, p["Domain Extent"], p["Resolution"], p["Dt"]/p["Sub Steps"], diffusivity=nu)
...
def get_korteweg_de_vries(is_test_set:bool, seed:int) -> tuple[dict, dict, ex.BaseStepper, jnp.ndarray]:
    ...
    stepper = ex.stepper.KortewegDeVries(3, extent, p["Resolution"], p["Dt"]/p["Sub Steps"], diffusivity=nu)
...
def get_kuramoto_sivashinsky(is_test_set:bool, seed:int) -> tuple[dict, dict, ex.BaseStepper, jnp.ndarray]:
    ...
    extent = np.random.uniform(p["Domain Extent (min)"], p["Domain Extent (max)"])
    stepper = ex.stepper.KuramotoSivashinsky(3, extent, p["Resolution"], p["Dt"]/p["Sub Steps"])
...
def get_fisher_kpp(is_test_set:bool, seed:int) -> tuple[dict, dict, ex.BaseStepper, jnp.ndarray]:
    ...
    stepper = ex.stepper.reaction.FisherKPP(3, p["Domain Extent"], p["Resolution"], p["Dt"]/p["Sub Steps"], diffusivity=nu, reactivity=r)
...
def get_gray_scott(is_test_set:bool, seed:int, variant:str) -> tuple[dict, dict, ex.BaseStepper, jnp.ndarray]:
    ...
    ic_gen = ex.ic.RandomMultiChannelICGenerator([
        ex.ic.RandomGaussianBlobs(3, domain_extent=p["Domain Extent"],
    ...
    stepper = ex.stepper.reaction.GrayScott(3, p["Domain Extent"], p["Resolution"], p["Dt"]/p["Sub Steps"],
...
def get_swift_hohenberg(is_test_set:bool, seed:int) -> tuple[dict, dict, ex.BaseStepper, jnp.ndarray]:
    ...
    stepper = ex.stepper.reaction.SwiftHohenberg(3, p["Domain Extent"], p["Resolution"], p["Dt"]/p["Sub Steps"], reactivity=r, critical_number=k)
