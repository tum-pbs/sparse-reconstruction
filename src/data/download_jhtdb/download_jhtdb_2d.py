...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Download 2D PDE simulation data")
    parser.add_argument("--dataset_name", type=str, default="channel")
    parser.add_argument("--out_name", type=str, default="channel")
    parser.add_argument("--out_path", type=str, default="/mnt/ssdraid/pbdl-datasets-local/2d_jhtdb_new")
...
def prepare_data_and_log(json_path_log, h5py_path, p):
    ...
    with h5py.File(h5py_path, "a") as h5py_file:
        ...
        for i in range(len(p["Z Slices"])):
            if not "sims/sim%d" % i in h5py_file:
                ...
                print("Created empty dataset with shape (%d, %d, %d, %d)" % (time_steps, channels, res_x, res_y))
                h5py_file.create_dataset("sims/sim%d" % i, shape=(time_steps, channels, res_x, res_y), dtype=np.float32)
...
def core_download(token, data_set, field_code, t, start, end, step):
    ...
    print("Downloaded %s slice %04d at time step %d with shape %s" % (data_set, start[2], t, str(result.shape)))
...
def download_data(
    ...
    download_tries = 50
    timeout_min = 2 if dataset_name in ["mhd1024", "channel"] else 1
    ...
    # main download
    data = []
    for t in range(p["Temporal Start"], p["Temporal End"]+1, p["Temporal Step"]):
        # check if timestep already exists
        with h5py.File(h5py_path, "r") as h5py_file:
            any_empty = False
            for sim in range(len(p["Z Slices"])):
                data = h5py_file["sims/sim%d" % sim][ (t-1) // p["Temporal Step"]]
                if not np.any(data): # ensure that the timestep is not empty, since h5py intializes with zeros
                    any_empty = True
        ...
        def handle_result(result):
            z_slices.append(result)
        def handle_error(error):
            print("\n\nError occoured:")
            print(error)

        for tries in range(download_tries):
            # download all 2d slices (in parallel) as the JHTDB API does not permit large 3d cutouts
            pool = mp.Pool(workers, maxtasksperchild=1)
            for z_slice in p["Z Slices"]:
                if z_slice in [s[0] for s in z_slices]:
                    continue

                slice_start = np.array([start[0], start[1], z_slice]).astype(np.int32)
                slice_end = np.array([end[0], end[1], z_slice]).astype(np.int32)
                slice_step = np.array([step[0], step[1], 1]).astype(np.int32)
            ...
            waited_min = 0
            while len(z_slices) < len(p["Z Slices"]) and waited_min < timeout_min:
            ...
            pool.close()
            if len(z_slices) >= len(p["Z Slices"]):
                pool.join()
                break
            else:
        ...
        with h5py.File(h5py_path, "a") as h5py_file:
            for i in range(len(z_slices)):
                ...
                data = np.transpose(data, (2,1,0)) # move channels to the front, jhtdb also transposes axes

                h5py_file["sims/sim%d" % p["Z Slices"].index(z)][ (t-1) // p["Temporal Step"]] = data

                print("Timestep %d (index: %d) at z-slice %d with shape %s written to disk\n" % (t, (t-1)//p["Temporal Step"], z, str(data.shape)))
    ...
    with h5py.File(h5py_path, "r") as h5py_file:
        for i in range(len(p["Z Slices"])):
            ...
            for t in range(0, time_steps, time_steps // steps_plot):
                data.append(h5py_file["sims/sim%d" % i][t])
