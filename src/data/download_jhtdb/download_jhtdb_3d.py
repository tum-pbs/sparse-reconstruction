...
def prepare_data_and_log(json_path_log, h5py_path, p):
    ...
    with h5py.File(h5py_path, "a") as h5py_file:
        ...
        if not "sims/sim0" in h5py_file:
            ...
            h5py_file.create_dataset("sims/sim0", shape=(time_steps, channels, res_x, res_y, res_z), dtype=np.float32)
...
def download_data(
    ...
    for t in range(p["Temporal Start"], p["Temporal End"]+1, p["Temporal Step"]):
        ...
        with h5py.File(h5py_path, "r") as h5py_file:
            data = h5py_file["sims/sim0"][ (t-1) // p["Temporal Step"]]
        ...
        with h5py.File(h5py_path, "a") as h5py_file:
            ...
            h5py_file["sims/sim0"][ (t-1) // p["Temporal Step"]] = data
