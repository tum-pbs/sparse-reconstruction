...
class PosEmbMLPSwinv2D(nn.Module):
    def __init__(self,
        ...
        # print('Relative Coords Table: ', relative_coords_table)
        ...
        # print('Relative Position Index: ', relative_position_index)
...
class FasterDiTImpl(nn.Module):
    ...
    def __init__(
        ...
        for i in range(self.num_encoder_layers):
            hidden_size_layer = min(hidden_size * 2 ** i, max_hidden_size)
            self.__setattr__(f"encoder_level_{i}", FasterDiTStage(dim=hidden_size_layer, num_heads=num_heads,
        ...
        self.latent = FasterDiTStage(dim=hidden_size_latent, num_heads=num_heads,
        ...
        self.__setattr__("decoder_level_0", FasterDiTStage(dim=hidden_size_layer0, num_heads=num_heads,
        ...
        for i in range(1, self.num_encoder_layers):
            ...
            self.__setattr__(f"decoder_level_{i}", FasterDiTStage(dim=hidden_size_layer, num_heads=num_heads,
...
class WrapperModel2D(ModelMixin, ConfigMixin):
    ...
    def __init__(self, model: Dict,
        ...
        self.encoder = ConditionedEncoder2D(
        ...
        self.label_embed = LabelEmbedder(num_classes=num_classes,
                                            hidden_size=cond_embedding_dim,
                                            dropout_prob=class_dropout_prob)

        self.decoder = ConditionedDecoder2D(
            out_channels=out_channels,
            feature_embedding_dim=feature_embedding_dim,
            num_upsampling_layers=num_downsampling_layers,
            embedding_dim=cond_embedding_dim,
            features_first_layer=self.model_impl.out_channels,
            num_groups=num_groups,
        )
    ...
    def forward(
        ...
        timestep_embedding = get_timestep_embedding(timestep * 1000, self.cond_embedding_dim) # scaling from 0-1 to 0-1000
        class_label_embedding = self.label_embed(class_labels, self.training)
        conditioning = timestep_embedding + class_label_embedding

        residuals = self.encoder(hidden_states, conditioning)
        state = residuals[-1]

        state = self.model_impl(state, timestep, class_labels).sample

        out = self.decoder(state, conditioning, residuals)

        return SwinDiTOutput(sample=out)
...
def FasterDiT_S2(**kwargs):
    return FasterDiTImpl(down_factor=2, hidden_size=384, max_hidden_size=384, num_heads=4, depth=[2, 5, 8, 5, 2], ffn_type='rep', rep=1, mlp_ratio=4,
                 attn_type='v2', posemb_type='rope2d', downsampler='dwconv5', down_shortcut=1, **kwargs)
...
def FasterDiT_B2(**kwargs):
    return FasterDiTImpl(down_factor=2, hidden_size=768, max_hidden_size=768, num_heads=8, depth=[2, 5, 8, 5, 2], ffn_type='rep', rep=1, mlp_ratio=4,
                 attn_type='v2', posemb_type='rope2d', downsampler='dwconv5', down_shortcut=1, **kwargs)
...
def FasterDiT_L2(**kwargs):
    return FasterDiTImpl(down_factor=2, hidden_size=1024, max_hidden_size=1024, num_heads=16, depth=[2, 5, 8, 5, 2], ffn_type='rep', rep=1,
                 mlp_ratio=4, attn_type='v2', posemb_type='rope2d', downsampler='dwconv5', down_shortcut=1, **kwargs)
...
FasterDiT_models = {
    ...
    'FasterDiT-S2': FasterDiT_S2,  # U-DiT-S
    'FasterDiT-B2': FasterDiT_B2,  # U-DiT-B
    'FasterDiT-L2': FasterDiT_L2,  # U-DiT-L
}