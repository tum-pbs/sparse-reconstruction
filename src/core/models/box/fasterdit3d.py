...
from diffusers.models.embeddings import CombinedTimestepLabelEmbeddings, get_timestep_embedding
...
def Wrapper3D_S(channel_size: int,
    ...
    model = {
        'target': 'src.core.models.box.fasterdit3d.PlaceholderModel',
        'params': {
            'dim': feature_embedding_dim * (2 ** num_downsampling_layers)
        }
    }

    image_size = -1
    return WrapperModel3D(model, in_channels=channel_size, out_channels=channel_size_out, sample_size=image_size,
                          feature_embedding_dim=feature_embedding_dim, num_downsampling_layers=num_downsampling_layers,
                          time_embedding_dim=64, num_groups=feature_embedding_dim)



def FasterDiT3D_S(image_size: int, channel_size: int,
    ...
    model = {
        'target': 'src.core.models.box.FasterDiT3DBackbone',
        'params': {
            'dim': feature_embedding_dim * (2 ** num_downsampling_layers),
            'window_size': [4, 4, 4],  # [ 5, 5, 2, 2 ]
            'num_heads': [16, 16, 16],  # [ 16, 16, 16, 16 ]
            'depths': [2, 2, 2],  # [ 1, 2, 4, 2 ]
            'ct_size': 1,
            'mlp_ratio': 4,
            'resolution': int(image_size // (2 ** num_downsampling_layers)),
        }
    }
    ...
    return WrapperModel3D(model, in_channels=channel_size, out_channels=channel_size_out, sample_size=image_size,
                          ...
                          num_groups=feature_embedding_dim,
                          repetitions=2)

def FasterDiT3D_B(image_size, channel_size, **kwargs):
    ...
    return WrapperModel3D(model, in_channels=channel_size, out_channels=channel_size, sample_size=image_size,
                          feature_embedding_dim=feature_embedding_dim, num_downsampling_layers=num_downsampling_layers,
                          time_embedding_dim=64,
                          num_groups=feature_embedding_dim,
                          repetitions=2)

def FasterDiT3D_L(image_size, channel_size, **kwargs):
...
class WrapperModel3D(ModelMixin, ConfigMixin):
    ...
    def __init__(self, model: Dict,
        ...
        self.latent_size = self.model_impl.num_features_out
    ...
    def encode(self,
        ...
        _, state = self.model_impl(state, timestep, class_labels)

        residuals[-1] = state

        return Output3D(hidden_states=residuals, embedding=emb)
    ...
    def forward(
        ...
        encoded = self.encode(hidden_states, timestep, class_labels)
        state = encoded.hidden_states[-1]

        out = self.decoder(state, encoded.embedding, encoded.hidden_states)

        return Output3D(reconstructed=out, sample=out)

class FasterDiT3DBackbone(ModelMixin, ConfigMixin):
    """
