...
from transformers.models.swinv2 import Swinv2Config
...
def window_partition(input_feature, window_size):
    ...
    """
    batch_size, height, width, depth, num_channels = input_feature.shape
    input_feature = input_feature.view(
        batch_size, height // window_size, window_size, width // window_size, window_size, depth // window_size,
        window_size, num_channels
    )
    windows = input_feature.permute(0, 1, 3, 5, 2, 4, 6, 7).contiguous().view(-1, window_size, window_size, window_size,
                                                                              num_channels)
    return windows


# Copied from transformers.models.swin.modeling_swin.window_reverse
def window_reverse(windows, window_size, height, width, depth):
    """
    Merges windows to produce higher resolution features.
    """
    num_channels = windows.shape[-1]
    windows = windows.view(-1, height // window_size, width // window_size, depth // window_size, window_size, window_size, window_size, num_channels)
    windows = windows.permute(0, 1, 4, 2, 5, 3, 6, 7).contiguous().view(-1, height, width, depth, num_channels)
    return windows
...
class Swinv2Embeddings(nn.Module):
    ...
    def interpolate_pos_encoding(self, embeddings: torch.Tensor, height: int, width: int, depth: int) -> torch.Tensor:
        ...
        sqrt_num_positions = int(cube_root(num_positions))
        patch_pos_embed = patch_pos_embed.reshape(1, sqrt_num_positions, sqrt_num_positions, sqrt_num_positions, dim) # Not sure
    ...
    def forward(
        self,
        voxel_values: Optional[torch.FloatTensor],
        bool_masked_pos: Optional[torch.BoolTensor] = None,
        interpolate_pos_encoding: bool = False,
    ) -> Tuple[torch.Tensor, Tuple[int, int, int]]:
        _, num_channels, height, width, depth = voxel_values.shape
        embeddings, output_dimensions = self.patch_embeddings(voxel_values)
...
class Swinv2PatchEmbeddings(nn.Module):
    ...
    def maybe_pad(self, voxel_values, height, width, depth):
        if depth % self.patch_size[2] != 0:
            pad_values = (0, self.patch_size[2] - depth % self.patch_size[2])
            voxel_values = nn.functional.pad(voxel_values, pad_values)
        if width % self.patch_size[1] != 0:
            pad_values = (0, 0, 0, self.patch_size[1] - width % self.patch_size[1])
            voxel_values = nn.functional.pad(voxel_values, pad_values)
    ...
    def forward(self, pixel_values: Optional[torch.FloatTensor]) -> Tuple[torch.Tensor, Tuple[int, int, int]]:
        _, num_channels, height, width, depth = pixel_values.shape
        # pad the input to be divisible by self.patch_size, if needed
        pixel_values = self.maybe_pad(pixel_values, height, width, depth)
        embeddings = self.projection(pixel_values)
        _, _, height, width, depth = embeddings.shape
        output_dimensions = (height, width, depth)
...
class Swinv2PatchMerging(nn.Module):
    ...
    def __init__(self, input_resolution: Tuple[int, int, int], dim: int, norm_layer: nn.Module = nn.LayerNorm) -> None:
        super().__init__()
        self.input_resolution = input_resolution
        self.dim = dim
        self.reduction = nn.Linear(8 * dim, 2 * dim, bias=False) # Changed
        self.norm = norm_layer(2 * dim)

    def maybe_pad(self, input_feature, height, width, depth):
        should_pad = (height % 2 == 1) or (width % 2 == 1) or (depth % 2 == 1)
        if should_pad:
            pad_values = (0, depth % 2, 0, width % 2, 0, height % 2)
            input_feature = nn.functional.pad(input_feature, pad_values)

        return input_feature

    def forward(self, input_feature: torch.Tensor, input_dimensions: Tuple[int, int, int]) -> torch.Tensor:
        height, width, depth = input_dimensions
        # `dim` is height * width * depth
        batch_size, dim, num_channels = input_feature.shape

        input_feature = input_feature.view(batch_size, height, width, depth, num_channels)
        # pad input to be disible by width and height, if needed
        input_feature = self.maybe_pad(input_feature, height, width, depth)

        # Perform 2x2x2 merging
        input_feature_0 = input_feature[:, 0::2, 0::2, 0::2, :]
        input_feature_1 = input_feature[:, 1::2, 0::2, 0::2, :]
        input_feature_2 = input_feature[:, 0::2, 1::2, 0::2, :]
        input_feature_3 = input_feature[:, 1::2, 1::2, 0::2, :]
        input_feature_4 = input_feature[:, 0::2, 0::2, 1::2, :]
        input_feature_5 = input_feature[:, 1::2, 0::2, 1::2, :]
        input_feature_6 = input_feature[:, 0::2, 1::2, 1::2, :]
        input_feature_7 = input_feature[:, 1::2, 1::2, 1::2, :]

        # Concatenate along the channel dimension
        input_feature = torch.cat(
            [input_feature_0, input_feature_1, input_feature_2, input_feature_3, input_feature_4, input_feature_5, input_feature_6, input_feature_7],
            -1
        )
        input_feature = input_feature.view(batch_size, -1, 8 * num_channels)  # [batch_size, (H/2 * W/2 * D/2), 8*C]
...
class Swinv2SelfAttention(nn.Module):
    def __init__(self, config, dim, num_heads, window_size, pretrained_window_size=[0, 0, 0]):
        super().__init__()
        if dim % num_heads != 0:
            raise ValueError(
        ...
        relative_coords_d = torch.arange(-(self.window_size[2] - 1), self.window_size[2], dtype=torch.int64).float()
        relative_coords_table = (
            torch.stack(meshgrid([relative_coords_h, relative_coords_w, relative_coords_d], indexing="ij"))
            .permute(1, 2, 3, 0)
            .contiguous()
            .unsqueeze(0)
        ) # 1, 2*Wh-1, 2*Ww-1, 2*Wd-1, 3
        ...
        )  # 3, Wh*Ww*Wd, Wh*Ww*Wd, Wh*Ww*Wd
        relative_coords = relative_coords.permute(
            1, 2, 0
        ).contiguous()  # Wh*Ww*Wd, Wh*Ww*Wd, 2
...
class Swinv2Attention(nn.Module):
    def __init__(self, config, dim, num_heads, window_size, pretrained_window_size=0):
        ...
        self.self = Swinv2SelfAttention(
            ...
            else (pretrained_window_size, pretrained_window_size, pretrained_window_size),
        )
...
class Swinv2Layer(nn.Module):
    def __init__(
        ...
        window_size, shift_size = self._compute_window_shift(
            (config.window_size, config.window_size, config.window_size), (shift_size, shift_size, shift_size)
        )
    ...
    def _compute_window_shift(self, target_window_size, target_shift_size) -> Tuple[Tuple[int, int, int], Tuple[int, int, int]]:
        window_size = [r if r <= w else w for r, w in zip(self.input_resolution, target_window_size)]
        shift_size = [0 if r <= w else s for r, w, s in zip(self.input_resolution, window_size, target_shift_size)]
        return window_size, shift_size

    def get_attn_mask(self, height, width, depth, dtype):
        if self.shift_size > 0:
            # calculate attention mask for shifted window multihead self attention
            img_mask = torch.zeros((1, height, width, depth, 1), dtype=dtype)
            height_slices = (
    ...
    def maybe_pad(self, hidden_states, height, width, depth):
        ...
        pad_values = (0, pad_depth, 0, pad_right, 0, pad_bottom)
    ...
    def forward(
        ...
        hidden_states = hidden_states.view(batch_size, height, width, depth, channels)
        hidden_states, pad_values = self.maybe_pad(hidden_states, height, width, depth)
        _, height_pad, width_pad, depth_pad, _ = hidden_states.shape
        # cyclic shift
        if self.shift_size > 0:
            shifted_hidden_states = torch.roll(hidden_states, shifts=(-self.shift_size, -self.shift_size, -self.shift_size), dims=(1, 2, 3))
        else:
            shifted_hidden_states = hidden_states

        # partition windows
        hidden_states_windows = window_partition(shifted_hidden_states, self.window_size)
        hidden_states_windows = hidden_states_windows.view(-1, self.window_size * self.window_size * self.window_size, channels)
        attn_mask = self.get_attn_mask(height_pad, width_pad, depth_pad, dtype=hidden_states.dtype)
        ...
        attention_windows = attention_output.view(-1, self.window_size, self.window_size, self.window_size, channels)
        shifted_windows = window_reverse(attention_windows, self.window_size, height_pad, width_pad, depth_pad)

        # reverse cyclic shift
        if self.shift_size > 0:
            attention_windows = torch.roll(shifted_windows, shifts=(self.shift_size, self.shift_size, self.shift_size), dims=(1, 2, 3))
        else:
            attention_windows = shifted_windows

        was_padded = pad_values[1] > 0 or pad_values[3] > 0 or pad_values[5] > 0
        if was_padded:
            attention_windows = attention_windows[:, :height, :width, :depth, :].contiguous()
...
class Swinv2Stage(nn.Module):
    ...
    def forward(
        ...
        if self.downsample is not None:
            height_downsampled, width_downsampled, depth_downsampled = (height + 1) // 2, (width + 1) // 2, (depth + 1) // 2
            output_dimensions = (height, width, depth, height_downsampled, width_downsampled, depth_downsampled)
            hidden_states = self.downsample(hidden_states_before_downsampling, input_dimensions)
        else:
            output_dimensions = (height, width, depth, height, width, depth)
...
class Swinv2Decoder(nn.Module):
    ...
    def __init__(self, config):
        ...
        #     PixelShuffle3d(upsample_factor),
        # )

        self.config = config

        # Decoder blocks: One for each skip connection
        self.decoder_blocks = nn.ModuleList()

        encoder_stride = 2
        encoder_dims = [self.config.embed_dim * (2 ** i) for i in range(len(self.config.depths))]

        # reversed channel dimensions for decoding
        decoder_dims = encoder_dims[::-1]

        for channels in decoder_dims:
            self.decoder_blocks.append(
                nn.Sequential(
                    nn.Conv3d(channels, (encoder_stride ** 3 * channels) // 2, 1),  # Prepare for PixelShuffle
                    PixelShuffle3d(encoder_stride),  # Upsample by factor of 2
                )
            )

        self.final_layer = nn.Conv3d(self.config.embed_dim // 2, self.config.num_channels, 1)
    ...
    def forward(self, hidden_states: list[torch.Tensor]) -> torch.Tensor:
        ...
        for i, decoder_block in enumerate(self.decoder_blocks):
            if i + 1 < len(hidden_states):
                skip_connection = hidden_states[i + 1]
                x = x + skip_connection

            # Upsample with PixelShuffle3D
            x = decoder_block(x)
    ...
    def __init__(self, config, grid_size, pretrained_window_sizes=(0, 0, 0, 0)):
        ...
        for i_layer in range(self.num_layers):
            stage = Swinv2Stage(
                ...
                input_resolution=(grid_size[0] // (2**i_layer), grid_size[1] // (2**i_layer), grid_size[2] // (2**i_layer)),
...
class DenseSwin3D(Swinv2PreTrainedModel):
    def __init__(self, config, do_upsampling=True):
        super().__init__(config)

        self.do_upsampling = do_upsampling
        self.swinv2 = Swinv2Model(config, add_pooling_layer=False, use_mask_token=False)
        self.config = config
        self.latent_size = config.embed_dim * 2 ** (len(config.depths) - 1)
    ...
    def encode(self,
               ...
               pde_parameters: Optional[torch.Tensor] = None) -> Output3D:
        ...
        # SwinV2 has no time/class embedding
        embedding = torch.zeros(x.shape[0], dtype=x.dtype, device=x.device)

        return Output3D(hidden_states = out.reshaped_hidden_states,
                        embedding=embedding)

    def decode(self,
                x: torch.Tensor,
                embedding: torch.Tensor,
                residuals: List[torch.Tensor]) -> Output3D:

        hidden_states = list(residuals)
        x = self.decoder(hidden_states[::-1])

        return Output3D(reconstructed=x,
                        sample=x)
    ...
    def forward(
        ...
        encoded = self.encode(x, timestep, class_labels)
        ...
        #
        # # Decoder path
        # hidden_states = outputs.reshaped_hidden_states[::-1]  # Reverse for decoder
        # x = hidden_states[0]                                  # Start with bottleneck
        #
        # if self.do_upsampling:
        #     x = self.decoder(hidden_states)
        #     reconstructed_pixel_values = x
        # else:
        #     reconstructed_pixel_values = x
        #
    ...
    def init_decoder(self):
        ...
        self.decoder = Swinv2Decoder(self.config)
...
def DenseSwin3D_custom(image_size, channel_size, **kwargs):

    config_dict = {
        "image_size": image_size,  # Adjusted to smaller resolution
        "num_channels": channel_size,  # Number of input channels
        "patch_size": 2,  # Size of the patch
        "embed_dim": 96,  # Embedding dimension
        "depths": [2, 2, 4],  # Number of layers per stage
        "num_heads": [12, 12, 12],  # Number of attention heads per stage
        "window_size": 4,  # Window size for attention
        "mlp_ratio": 4.0,  # MLP expansion ratio
        "drop_path_rate": 0.0,  # Stochastic depth rate
        "use_absolute_embeddings": True,  # Use absolute position embeddings
        "qkv_bias": True,
    ...
    return DenseSwin3D(config, do_upsampling=True)


def DenseSwin3D_S(image_size, channel_size, **kwargs):
    ...
    config = Swinv2Config(
        ...
        num_heads=[12, 12, 12, 12],     # Number of attention heads per stage
        window_size=4,                  # Window size for attention
        mlp_ratio=4.0,                  # MLP expansion ratio
        drop_path_rate=0.0,             # Stochastic depth rate
        use_absolute_embeddings=True,   # Use absolute position embeddings
...
def DenseSwin3D_B(image_size, channel_size, **kwargs):
    config = Swinv2Config(
        image_size=image_size,  # Adjusted to smaller resolution
        num_channels=channel_size,  # Number of input channels
        patch_size=2,  # Size of the patch
        ...
        depths=[2, 2, 2, 2],  # Number of layers per stage
        num_heads=[16, 16, 16, 16],  # Number of attention heads per stage
        window_size=4,  # Window size for attention
        mlp_ratio=4.0,  # MLP expansion ratio
        drop_path_rate=0.0,  # Stochastic depth rate
        use_absolute_embeddings=True,  # Use absolute position embeddings
...
def DenseSwin3D_L(image_size, channel_size, **kwargs):
    config = Swinv2Config(
        image_size=image_size,  # Adjusted to smaller resolution
        num_channels=channel_size,  # Number of input channels
        patch_size=2,  # Size of the patch
...
DenseSwin3D_models = {
    "DenseSwin3D-S": DenseSwin3D_S,
    "DenseSwin3D-B": DenseSwin3D_B,
    "DenseSwin3D-L": DenseSwin3D_L,
}