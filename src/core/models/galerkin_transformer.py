...
### Implementation from https://github.com/scaomath/galerkin-transformer/blob/main/libs/model.py ###
...
from neuralop.layers.spectral_convolution import SpectralConv3d, SpectralConv2d, SpectralConv1d, SpectralConv
...
class SpectralRegressor(nn.Module):
    def __init__(self, in_dim,
        ...
        if spacial_dim == 3: #3d, function + (x,y,z)
            ...
            permute_to_channel_first = (0, 4, 1, 2, 3)
            permute_to_channel_last = (0, 2, 3, 4, 1)
        elif spacial_dim == 2:  # 2d, function + (x,y)
            spectral_conv = SpectralConv
            permute_to_channel_first = (0, 3, 1, 2)
            permute_to_channel_last = (0, 2, 3, 1)
        elif spacial_dim == 1:  # 1d, function + x
            spectral_conv = SpectralConv
            permute_to_channel_first = (0, 2, 1)
            permute_to_channel_last = (0, 2, 1)
        else:
            raise NotImplementedError(f"{spacial_dim}D not implemented.")
        activation = default(activation, 'silu')
        self.activation = nn.SiLU() if activation == 'silu' else nn.ReLU()
        dropout = default(dropout, 0.1)
        ...
        n_modes = tuple([modes] * spacial_dim)

        self.permute_to_channel_first = permute_to_channel_first
        self.permute_to_channel_last = permute_to_channel_last
    ...
    def forward(self, x, edge=None, pos=None, grid=None):
        ...
        x = x.permute(self.permute_to_channel_first)
        ...
        x = x.permute(self.permute_to_channel_last)
...
class FourierTransformer3DLite(nn.Module):
    '''
    A lite model of the Fourier/Galerkin Transformer for 3D
    '''

    def __init__(self, **kwargs):
        super(FourierTransformer3DLite, self).__init__()
        self.config = defaultdict(lambda: None, **kwargs)
        self._get_setting()
        self._initialize()
    ...
    def forward(self, node, edge, pos, grid=None):
        ...
        - pos: (batch_size, n*n*n, pos_dim)
        - grid: (batch_size, n, n, n, pos_dim)
        '''
        bsz = node.size(0)
        ...
        x = self.dpo(x)
        x = x.view(bsz, n_grid, n_grid, n_grid, -1)
        x = self.regressor(x, grid=grid)
...
class FourierTransformer2DLite(nn.Module):
    '''
    A lite model of the Fourier/Galerkin Transformer for 2D
    '''
...
@dataclass
class GalerkinTransformer2DOutput(BaseOutput):
    """
    The output of [`GalerkinTransformer2D`].
...
class GalerkinTransformer2D(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        self.model = FourierTransformer2DLite(**config)
    ...
    def forward(
        ...
        output = self.model(hidden_states, None, pos, grid)['preds']

        # transpose output dimensions
        output = output.permute(0, 3, 1, 2).contiguous()

        if not return_dict:
            return (output,)

        return GalerkinTransformer2DOutput(sample=output)

@dataclass
class GalerkinTransformer3DOutput(BaseOutput):
    """
    The output of [`GalerkinTransformer3D`].
...
class GalerkinTransformer3D(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        config = defaultdict(lambda: None,
                             node_feats=in_channels + 3,
                             pos_dim=3,
                             n_targets=in_channels,
                             n_hidden=n_hidden,  # attention's d_model
                             num_feat_layers=0,
                             ...
                             return_latent=False,
                             decoder_type='ifft',
                             freq_dim=freq_dim,  # hidden dim in the frequency domain
                             num_regressor_layers=num_regressor_layers,  # number of spectral layers
                             fourier_modes=fourier_modes,  # number of Fourier modes
                             spacial_dim=3,
                             spacial_fc=False,
                             dropout=0.0,
                             encoder_dropout=0.0,
        ...
        x = np.linspace(0, 1, self.n_grid)
        y = np.linspace(0, 1, self.n_grid)
        z = np.linspace(0, 1, self.n_grid)
        x, y, z = np.meshgrid(x, y, z)
        grid = np.stack([x, y, z], axis=-1)
        pos = np.c_[x.ravel(), y.ravel(), z.ravel()]

        self.register_buffer('grid', torch.tensor(grid, dtype=torch.float))
        self.register_buffer('pos', torch.tensor(pos, dtype=torch.float))

        self.model = FourierTransformer3DLite(**config)
    ...
    def forward(
        ...
        # (batch_dim, channels, height, width, depth) -> (batch_dim, height*width*depth, channels)
        batch_dim = hidden_states.size(0)
        hidden_states = hidden_states.view(batch_dim, -1, hidden_states.size(1))

        # add batch size to pos
        pos = self.pos.unsqueeze(0).expand(batch_dim, -1, -1)
        grid = self.grid.unsqueeze(0).expand(batch_dim, -1, -1, -1, -1)
        ...
        return GalerkinTransformer3DOutput(sample=output)
