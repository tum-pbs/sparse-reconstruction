...
import turb_datasets_pairs

def reg_standard_step(model, x, target, optimizer, config, accumulation_steps, batch_idx, length):
    # Forward pass
    pred = model(x)
    pred = pred.sample
...
def reg_PINN_step(dataset, model, x, target, optimizer, config):
    # Forward pass
    pred = model(x)
    pred = pred.sample
    loss = ((target - pred) ** 2).mean()
...
def reg_PINN_dyn_step(dataset, model, x, target, optimizer, config):
    # Forward pass
    pred = model(x)
    pred = pred.sample
    loss = ((target - pred) ** 2).mean()
...
def reg_ConFIG_step(dataset, model, x, target, optimizer, config, operator, accumulation_steps, batch_idx, length, grads1, grads2):
    optimizer.zero_grad()
    # Forward pass
    pred = model(x)
    pred = pred.sample
    ...
    # Compute the divergence-free loss
    divergence = utils.compute_divergence(dataset.Y_scaler.inverse(pred[:, :3, :, :, :]), 2*math.pi/config.Data.grid_size)
    divergence_loss = torch.mean(torch.abs(divergence)) / accumulation_steps
...
def train_regression(config):
    ...
    if config.Data.case == "iso":
        ...
        train_loader, val_loader, test_loader = turb_datasets_pairs.load_data_supervised(dataset_path="/mnt/data1/navarro/channel_point1", batch_size=config.Training.batch_size)
        dataset = None
    ...
    #model = Simple3DModel(in_channels=3)
    ...
    for epoch in range(config.Training.epochs):
        ...
        for batch_X, batch_Y in train_loader:
            ...
            if config.Training.method == "std":
                total_loss, loss = reg_standard_step(model, x, y, optimizer, config, accumulation_steps, batch_idx, len(train_loader))
                
            elif config.Training.method == "PINN":
                total_loss, loss = reg_PINN_step(dataset, model, x, y, optimizer, config)
                
            elif config.Training.method == "PINN_dyn":
                total_loss, loss = reg_PINN_dyn_step(dataset, model, x, y, optimizer, config)
                
            elif config.Training.method == "ConFIG":
                total_loss, loss = reg_ConFIG_step(dataset, model, x, y, optimizer, config, operator, accumulation_steps, batch_idx, len(train_loader), grads1, grads2)
        ...
        with torch.no_grad():
            for batch_X, batch_Y in val_loader:
                ...
                if config.Data.case == "iso":
                    eq_residual = utils.compute_divergence(dataset.Y_scaler.inverse(pred[:, :3, :, :, :]), 2*math.pi/config.Data.grid_size)
                elif config.Data.case == "channel":
                    eq_residual = utils.compute_divergence_fdm_wall_high_order(pred[:, :3, :, :, :], hx=2 * math.pi / 192, Ny=65, hz = math.pi / 192)
