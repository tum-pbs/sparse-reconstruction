...
def ddpm_standard_step(dataset, model, diffusion, y, optimizer, config, accumulation_steps, batch_idx, length):
    ...
    mse_loss = (noise - e_pred).square().mean() / accumulation_steps
    ...
    if config.Data.case == "iso":
        ...
        eq_residual = utils.compute_divergence_fdm_wall_high_order(x0_pred[:, :3, :, :, :], hx=2 * math.pi / 192, Ny=65, hz = math.pi / 192)
...
def ddpm_PINN_step(dataset, model, diffusion, y, optimizer, config, accumulation_steps, batch_idx, length):
    ...
    if config.Data.case == "iso":
        ...
        eq_residual = utils.compute_divergence_fdm_wall_high_order(x0_pred[:, :3, :, :, :], hx=2 * math.pi / 192, hy = 1 / 65, hz = math.pi / 192)
    eq_res_m = torch.mean(torch.abs(eq_residual)) / accumulation_steps
...
def ddpm_PINN_dyn_step(dataset, model, diffusion, y, optimizer, config, accumulation_steps, batch_idx, length):
    ...
    if config.Data.case == "iso":
        ...
        eq_residual_gt = utils.compute_divergence_fdm_wall_high_order(y[:, :3, :, :, :], hx=2 * math.pi / 192, Ny=65, hz = math.pi / 192)
...
def train_ddpm(config):
    ...
    #model = Simple3DDiffusionModel(in_channels=3)
    ...
    optimizer.zero_grad()
    operator = ConFIGOperator(length_model=ProjectionLength())
    ...
    for epoch in range(config.Training.epochs):
        ...
        for batch_idx, x1 in enumerate(train_loader):
            ...
            #print(mse_loss)
            physics_loss_epoch += physics_loss.item() #* accumulation_steps
            #print(physics_loss_epoch)
